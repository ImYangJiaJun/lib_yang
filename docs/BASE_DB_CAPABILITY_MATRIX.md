...
**基线日期**：2026-07-15  
**开发版本**：yang-base 0.1.3（待发布）/ yang-db 0.1.4
...
| 数据治理 | checksum migration、plan/dry-run、并发占位；`TableConfig` 只读验证与启动期 additive schema 同步 | MySQL 8 双实例并发、幂等补列、危险漂移拒绝 |
...
- `TableConfig` 不是完整 DDL source of truth；显式启动同步只创建缺失表/列/主键/索引，冲突时 fail-fast。
