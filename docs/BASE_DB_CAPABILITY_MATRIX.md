...
**基线日期**：2026-07-15  
**发布版本**：yang-base 0.1.2 / yang-db 0.1.4
...
| 范围 | 当前能力 | 约束与验证 |
|---|---|---|
| Action / Router | 类型化输入输出、schema、transport-neutral `RequestMeta`、确定性 `ApiCatalog`、可选 OpenAPI 3.1 | 编译失败测试、snapshot、路由纵向契约 |
| 插件 | 依赖拓扑、生命周期、配置 schema、迁移声明 | 循环/缺失依赖、配置反例与迁移漂移测试 |
| 数据治理 | checksum migration、plan/dry-run、并发占位；`TableConfig` 对真实 schema 的只读验证 | MySQL 8 真实并发、漂移和零 DDL 验证 |
...
| 运维可见性 | 错误分类、慢查询 tracing、敏感字段脱敏、可选 metrics | 日志捕获、故障注入和 Debug 泄露反例 |
| 后台扩展 | `admin-metadata` 保存稳定 ID 和展示元数据 | 零新增依赖，不持有或改变核心 dispatch |
...
- `TableConfig` 是运行期业务元数据，不是完整 DDL source of truth；schema 验证只报告差异，不自动修改生产表。
...
- 通用 ORM、任意 SQL AST、自动 schema migration：不在当前职责内。
