...
- [-] 1. 基础设施：错误体系改造与依赖结构调整
  ...
  - [x] 1.3 改造 BaseError 数据库/HTTP/Token 变体使用 `#[source]` 持有底层错误
  ...
  - [x] 1.4 完善 `From<yang_db::DbError>` 分类映射
  ...
  - [x] 1.5 新增 `From<jsonwebtoken::errors::Error>` 实现
...
- [-] 2. 优先级 1：无外部依赖的独立修复
  - [x] 2.1 删除 `table_query_select.rs` 孤儿文件
  ...
  - [x] 2.2 引入 `BUILTIN_ACTION_NAMES` 常量
  ...
  - [x] 2.4 实现 `quote_identifier` 与字段名白名单校验
  ...
  - [x] 2.6 重构 `append_where_to_sql` 统一 SQL 拼接
  ...
  - [x] 2.8 增强 `Validator::Email`/`Validator::Phone` 严格化与正则缓存
...
- [-] 3. 锁中毒与时钟异常处理
  - [x] 3.1 实现 `current_unix_timestamp` 辅助函数
  ...
  - [x] 3.2 处理 `HttpClient` 与 `GlobalTools` 锁中毒
...
- [-] 5. ApiResponse 错误传播
  - [x] 5.1 改造 `ApiResponse::success` 为 `Result<Self, BaseError>`
...
- [-] 6. ActionContext 参数访问与借用优化
  - [x] 6.1 添加 `path_param`/`query_param`/`param_or` 方法
  ...
  - [x] 6.2 添加 `param_optional_strict` 方法
  ...
  - [x] 6.3 优化 `ActionContext` 用户角色借用
    - 添加 `user_roles_slice(&self) -> &[String]` 方法返回借用而非克隆
    - _Requirements: 24.3_

  - [x] 6.4 重构 `TableQuery::new` 接受 `Arc<[String]>`
...
- [-] 7. DynamicRow 与内置 Action 修复
  ...
  - [x] 7.3 实现 `parse_paging_param` 安全转换辅助函数
...
- [x] 8. ModuleRouter 改进
  - [x] 8.1 `register_builtin_actions` 改为 `Result` 返回
  ...
  - [x] 8.2 重命名 `table_config` getter 解决命名冲突
...
- [x] 11. GlobalTools 全局单例
  - [x] 11.1 实现 `GlobalTools::init` 与 `GlobalTools::get`
...
- [ ] 12. HttpClient 配置与请求构建器改进
  ...
  - [x] 12.2 改造 `init_global`/`global` 使用结构化错误
  ...
  - [x] 12.3 `RequestBuilder` header 错误累积
...
- [-] 13. PluginManager 依赖图与 JSON Schema
  - [x] 13.1 `PluginManagerBuilder::build` 检查依赖完整性
  ...
  - [x] 13.3 `PluginManager::validate_config` 集成 `jsonschema`
...
- [-] 14. yang-db 关联前置：参数化查询接口与 DatabaseInitializer
  - [x] 14.1 核对 `yang-db::Database` 参数化查询接口
  ...
  - [x] 14.2 改造 `DatabaseInitializer` 使用参数化查询
...
- [-] 15. yang-db 关联前置：Redis API 补齐与 GlobalRedis 改造
  - [x] 15.1 核对 `yang-db::RedisClient` API 完整性
  ...
  - [x] 15.4 暴露 Pipeline / Transaction 入口
...
- [-] 16. 测试基础设施与代码质量
  ...
  - [x] 16.3 测试中 `panic!` → `assert!(matches!(...))` 替换
