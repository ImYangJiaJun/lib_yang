...
| `RegistrationEmailVerification` / `EmailVerificationConfig` / `RegistrationEmailSender(Handle)` | `src/action/auth/email_verification.rs` | 一次性验证码：摘要存储、防枚举、原子单次消费（`consume`）与只验不消费（`verify_only`，共享错误计数/上限销毁）；投递经 trait 注入 |
