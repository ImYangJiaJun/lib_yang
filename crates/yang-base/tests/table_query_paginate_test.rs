...
/// 创建测试用的表配�?fn create_test_table_config() -> Arc<TableConfig> {
    Arc::new(
...
async fn test_paginate_basic() {
    ...
    let query = 
    $tableConfig = //! TableQuery 分页查询集成测试
//!
//! 测试 paginate 方法的完整功能，包括�?//! - 执行 COUNT 查询获取总记录数
//! - 计算 LIMIT �?OFFSET
//! - 执行数据查询
//! - 构建 PaginatedResult 返回结果
//!
//! **验证需�?*: 5.7
//!
//! **注意**: 这些测试需�?Docker 环境。如果没�?Docker，测试将被跳过�?//! 运行测试：`cargo test --test table_query_paginate_test -- --test-threads=1 --ignored`
...
async fn test_paginate_basic() {
    ...
    let query = TableQuery::new(table_config, vec!["user".to_string()], Some(Arc::new(pool)));

    // 执行分页查询：第 1 页，每页 10 �?    let result: PaginatedResult<TestUser> = query.page(1, 10).unwrap().paginate().await.unwrap();

    // 验证结果
    assert_eq!(result.total, 50, "总记录数应该�?50");
    assert_eq!(result.page, 1, "当前页应该是 1");
    assert_eq!(result.page_size, 10, "每页大小应该�?10");
    assert_eq!(result.total_pages, 5, "总页数应该是 5");
    assert_eq!(result.data.len(), 10, "当前页数据条数应该是 10");
    assert!(result.has_next(), "应该有下一�?);
    assert!(!result.has_prev(), "不应该有上一�?);
}
...
async fn test_paginate_second_page() {
    ...
    // 执行分页查询：第 2 页，每页 10 �?    let result: PaginatedResult<TestUser> = query.page(2, 10).unwrap().paginate().await.unwrap();
...
async fn test_paginate_last_page() {
    ...
    // 执行分页查询：第 5 页（最后一页），每�?10 �?    let result: PaginatedResult<TestUser> = query.page(5, 10).unwrap().paginate().await.unwrap();
...
async fn test_paginate_empty_result() {
    ...
    // 验证结果
    assert_eq!(result.total, 0, "总记录数应该�?0");
...
async fn test_paginate_with_where_condition() {
    ...
    // 验证所有返回的记录都满足条�?    for user in &result.data {
...
async fn test_paginate_with_order_by() {
    ...
    assert_eq!(result.data[0].id, 50, "第一条记录的 id 应该�?50");
    assert_eq!(result.data[9].id, 41, "第十条记录的 id 应该�?41");
...
async fn test_paginate_with_field_selection() {
    ...
    // 执行分页查询：选择所有字�?    let result: PaginatedResult<TestUser> = query
    ...
    // 验证字段值存�?    for user in &result.data {
        assert!(user.id > 0, "id 应该大于 0");
        assert!(!user.name.is_empty(), "name 不应该为�?);
    }
}
...
async fn test_paginate_with_default_params() {
    ...
    // 执行分页查询：不设置分页参数，使用默认�?    let result: PaginatedResult<TestUser> = query.paginate().await.unwrap();

    // 验证结果：默认应该是�?1 页，每页 20 �?    assert_eq!(result.total, 50);
    assert_eq!(result.page, 1, "默认页码应该�?1");
    assert_eq!(result.page_size, 20, "默认每页大小应该�?20");
    assert_eq!(result.total_pages, 3, "总页数应该是 3");
    assert_eq!(result.data.len(), 20, "当前页数据条数应该是 20");
}
.Groups[1].Value
    $roles = //! TableQuery 分页查询集成测试
//!
//! 测试 paginate 方法的完整功能，包括�?//! - 执行 COUNT 查询获取总记录数
//! - 计算 LIMIT �?OFFSET
//! - 执行数据查询
