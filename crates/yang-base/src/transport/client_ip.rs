...
/// [`ActionContext`] 请求扩展中存放已验证客户端 IP 的键。
pub const CLIENT_IP_META_KEY: &str = "yang.client_ip";
    ...
    fn from_cidrs(values: &[String]) -> Result<Self, BaseError> {
        if values.len() > MAX_TRUSTED_PROXY_CIDRS {
            return Err(BaseError::ConfigError(format!(
                "受信代理 CIDR 最多允许 {MAX_TRUSTED_PROXY_CIDRS} 项"
            )));
        }
        let trusted_proxies = values
            ...
            .map(|(index, value)| {
                IpCidr::parse(value).map_err(|error| {
                    BaseError::ConfigError(format!("受信代理 CIDR 第 {index} 项无效: {error}"))
                })
            })
...
#[derive(Clone)]
pub struct TrustedClientIpMiddleware {
    resolver: TrustedClientIpResolver,
}

impl TrustedClientIpMiddleware {
    /// 从受信代理 CIDR 列表构建中间件；空列表表示忽略所有转发头。
    pub fn from_cidrs(values: &[String]) -> Result<Self, BaseError> {
        Ok(Self {
